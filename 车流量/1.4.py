...
#     # 用前一个时间点的数据填补（ffill）
#     group = group.ffill()
#     # 如果你只想填充 q 列，可以 group['q'] = group['q'].ffill()
...
# # 首先，确保数据按分组和时间正确排序
df_train_sorted = df_train.sort_values(['iu_ac', 't_1h'])
...
# 保存模型参数
torch.save(model.state_dict(), 'trained_fcnet2.pth')

# # 加载参数
# model.load_state_dict(torch.load('trained_fcnet2.pth'))
# model.eval()
...
# 预测前，先分组并排序一次
train_grouped = {k: v.sort_values('t_1h') for k, v in df_train_sorted.groupby('iu_ac')}
...
for row in df_test.itertuples():
    ...
    if group is not None:
        # 取 test_time 之前的所有数据
        past_data = group[group['t_1h'] < test_time]
        # 取最近24小时的最后48条数据
        last_seq = past_data['q'].values[-24:]
        if len(last_seq) == 24:
            pred_q = predict_next_q(last_seq)
        elif len(last_seq) > 0:
            pred_q = last_seq[-1]  # 或用均值等
        else:
            pred_q = 0
    else:
        pred_q = 0 # 标准化后平均值就是0
