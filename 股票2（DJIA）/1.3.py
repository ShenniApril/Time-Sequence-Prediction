...
# 只取数值型特征列
feature_cols = ['Date', 'Open', 'High', 'Low', 'Volume', 'Close', 'Adj Close']
# 取部分列
df_selected = df_train[feature_cols]
...
# 参数
SEQ_LEN = 10  # 用前24个时刻预测下一个
PRED_HOUR = 0  # 预测第0小时后的Close
...
df_sequences['y'] = train_val['Close'].shift(-PRED_HOUR).values
...
for epoch in range(EPOCHS):# 外层循环，代表了整个训练数据集的遍历次数
    ...
    for x, y in train_loader:# 内层每循环一次，就会进行一次梯度下降算法
        ...
        tCloseal_loss += loss.item() * x.size(0)
    train_loss = tCloseal_loss / len(train_dataset)
...
for idx, row in test.iterrows():
    ...
    if len(all_hist) >= SEQ_LEN:
        ...
        pred_Close = predict_next_Close(x_seq)
    else:
        pred_Close = 0
    estimate_Closes.append(pred_Close)
    

# 4. 反归一化
estimate_Closes = np.array(estimate_Closes) * std_dict['Close'] + mean_dict['Close']
test['Close_true'] = test['Close'] * std_dict['Close'] + mean_dict['Close']
test['Prev_Close'] = test['Close_true'].shift(1)

# 5. 计算指标
#计算MAE
# 假设 predicted_Closes 顺序和 test.index 顺序一致
mae = mean_absolute_error(test['Close_true'], estimate_Closes)
print(f"MAE: {mae:.4f}")
# 计算RMSE
rmse = root_mean_squared_error(test['Close_true'], estimate_Closes)
print(f"RMSE: {rmse:.4f}")

# 先构造前一天的Close
test['Prev_Close'] = test['Close_true'].shift(1)
# 真实涨跌方向
real_updown = (test['Close_true'].values > test['Prev_Close'].values).astype(int)
# 预测涨跌方向
pred_updown = (np.array(estimate_Closes) > test['Prev_Close'].values).astype(int)
# 计算准确率
...
real_updown = (test['Close_true'].values[mask] > test['Prev_Close'].values[mask]).astype(int)
pred_updown = (np.array(estimate_Closes)[mask] > test['Prev_Close'].values[mask]).astype(int)
real_return = (test['Close_true'].values[mask] - test['Prev_Close'].values[mask]) / test['Prev_Close'].values[mask]
